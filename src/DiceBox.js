...
class DiceBox {
    ...
    setDimensions(dimensions=undefined) {
        ...
        this.desk = new THREE.Mesh(
            new THREE.PlaneGeometry(this.display.containerWidth * 6, this.display.containerHeight * 6, 1, 1),
            shadowplane
        );
    ...
    eventCollide({ body, target }) {
        ...
        if (body.mass > 0) {
            ...
            if (body.diceShape === "d2") {
                ...
                sound =
                    this.sounds_dice[this.sound_dieMaterial][
                        Math.floor(Math.random() * this.sounds_dice[this.sound_dieMaterial].length)
                    ];
            }
            if (sound) {
    ...
    getDiceResults(id) {
        if (id !== undefined) {
            ...
            return {
                type: diceMesh.shape,
                sides: parseInt(diceMesh.shape.substring(1)),
                id,
                ...diceMesh.result.at(-1),
                position: {
                    x: position.x,
                    y: position.y,
                    z: position.z
