...
class DiceBox {
    constructor(element_container, options = {}) {
        ...
        this.container =
            element_container instanceof HTMLElement ? element_container : document.querySelector(element_container);
    ...
    async loadSounds() {
        ...
        if (!this.sounds_table.hasOwnProperty(this.surface)) {
            ...
            for (let s = 1; s <= numsounds; ++s) {
                const clip = await loadSound(this.assetPath + "sounds/surfaces/surface_" + this.surface + s + ".mp3");
        ...
        if (!this.sounds_dice.hasOwnProperty(this.sound_dieMaterial)) {
            ...
            for (let s = 1; s <= numsounds; ++s) {
                const clip = await loadSound(this.assetPath + "sounds/dicehit/dicehit_" + this.sound_dieMaterial + s + ".mp3");
    ...
    setDimensions(dimensions = undefined) {
        ...
        this.display.scale =
            Math.sqrt(this.display.containerWidth * this.display.containerWidth +
                this.display.containerHeight * this.display.containerHeight) / 13;
        this.makeWorldBox();
    ...
    swapDiceFace(dicemesh, result) {
        ...
        if (dicemesh.notation.type == "d10" && value == 0)
            value = 10;
        if (dicemesh.notation.type == "d100" && value == 0)
            value = 100;
        if (dicemesh.notation.type == "d100" && value > 0 && value < 10)
            value *= 10;
        if (dicemesh.notation.type == "d10" && result == 0)
            result = 10;
        if (dicemesh.notation.type == "d100" && result == 0)
            result = 100;
    ...
    eventCollide({ body, target }) {
        ...
        if ((this.lastSoundStep == body.world.stepnumber || this.lastSound > now) && currentSoundType != "dice")
            return;
        // also skip if it's too early and both last sound and this sound are the same
        if ((this.lastSoundStep == body.world.stepnumber || this.lastSound > now) &&
        ...
        if (body.mass > 0) {
            ...
            if (body.diceShape === "d2") {
                ...
                sound =
                    this.sounds_dice[this.sound_dieMaterial][Math.floor(Math.random() * this.sounds_dice[this.sound_dieMaterial].length)];
            }
            if (sound) {
    ...
    animateThrow(threadid, callback) {
        ...
        if (this.running == threadid && this.throwFinished()) {
            ...
            this.rolling = false;
            if (callback)
                callback.call(this, this.notationVectors);
    ...
    getDiceResults(id) {
        if (id !== undefined) {
            ...
            const position = diceMesh.position;
            const screenPosition = this.getScreenPosition(position);
            return Object.assign(Object.assign({ type: diceMesh.shape, sides: parseInt(diceMesh.shape.substring(1)), id }, diceMesh.result.at(-1)), { position: {
                    x: position.x,
                    y: position.y,
                    z: position.z
                }, screenPosition, scale: diceMesh.scale.x });
        }
        let counter = 0;
        ...
        return {
            ...
            sets: this.notationVectors.set.map((set) => {
                ...
                for (let index = counter; index <= endCount; index++) {
                    ...
                    const diceMesh = this.diceList[counter];
                    const position = diceMesh.position;
                    const screenPosition = this.getScreenPosition(position);
                    rolls.push(Object.assign(Object.assign({ type: set.type, sides: parseInt(set.type.substring(1)), id: counter }, diceMesh.result.at(-1)), { position: {
                            x: position.x,
                            y: position.y,
                            z: position.z
                        }, screenPosition, scale: diceMesh.scale.x }));
    ...
    async add(notationSting) {
        ...
        if (!dieCount)
            return this.roll(notationSting);
    ...
    onMouseMove(event) {
        ...
        if (intersects.length > 0) {
            ...
            if (this.hoveredDice !== intersectedDice) {
                ...
                const screenPosition = this.getScreenPosition(position);
                const eventData = Object.assign(Object.assign({}, diceInfo), { position: {
                        x: position.x,
                        y: position.y,
                        z: position.z
                    }, screenPosition, scale: intersectedDice.scale.x });
    ...
    onMouseClick(event) {
        ...
        const eventData = Object.assign(Object.assign({}, diceInfo), { position: {
                ...
                y: position.y,
                z: position.z
            }, screenPosition, scale: this.hoveredDice.scale.x });
        this.onDiceClick(eventData);
        const clickEvent = new CustomEvent("diceClick", { detail: eventData });
        document.dispatchEvent(clickEvent);
    }
    getScreenPosition(position) {
        // Project 3D position to screen coordinates if container is available
        if (this.container) {
            const rect = this.container.getBoundingClientRect();
